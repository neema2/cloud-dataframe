...
class TestPureRelationREPLIntegration(unittest.TestCase):
    ...
    def test_simple_select_with_repl(self):
        """Test a simple column selection with actual REPL integration."""
    ...
    def test_single_column_select_with_repl(self):
        """Test a single column selection with REPL integration."""
        with tempfile.TemporaryDirectory() as temp_dir:
            employee_data = [
                ["id", "name", "department_id", "salary"],
                [1, "Alice", 101, 75000],
            ...
            pure_query = f"#>{{local::DuckDuckDatabase.employees}}#->select(~name)"
            
            print(f"Executing in REPL: {pure_query}")
            
            repl_response = {
                "sql": "SELECT e.name FROM employees AS e"
            }
            ...
            expected_pure = "$employees->select(~name)"
            self.assertEqual(expected_pure, pure_code.strip())
            
            repl_syntax_expected = "#>{local::DuckDuckDatabase.employees}#->select(~name)"
            self.assertEqual(repl_syntax_expected, pure_query)
